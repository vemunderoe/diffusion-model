...
model = UNet(in_channels=3, out_channels=3, base_channels=64, embedding_dim=128, num_classes=len(train_dataset.classes)).to(device)
...
os.makedirs("checkpoints/cifar-10-linear-cfg", exist_ok=True)
...
for epoch in range(num_epochs):
    ...
    # Save checkpoint
    torch.save(model.state_dict(), f"checkpoints/cifar-10-linear-cfg/model_checkpoint_epoch_{epoch+1}.pth")    
    ...
    with torch.no_grad():        
        ...
        generated_samples = diffusion.sample_ddpm(x_shape=(1, 3, 32, 32), device=device, labels=single_label)                
        ...
        visualize_denoising_process(generated_samples, epoch=epoch, path="samples/generated_samples/cifar-10-linear-cfg", filename="samples_epoch", image_title="Denoising Process Visualization")


# Save the trained model
torch.save(model.state_dict(), "checkpoints/cifar-10-linear-cfg/model.pth")
