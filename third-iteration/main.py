...
#model = UNet(in_channels=3, out_channels=3, base_channels=128, embedding_dim=128).to(device)

print(f"Number of classes: {len(train_dataset.classes)}")
#model = EnhancedUNet(in_channels=3, out_channels=3, base_channels=128, embedding_dim=128, num_classes=len(train_dataset.classes)).to(device)
...
os.makedirs("checkpoints/mnist-linear-second", exist_ok=True)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (x, labels) in enumerate(train_loader):
        ...
        #predicted_noise = model(noisy_x, timesteps, None)
    ...
    # Save checkpoint
    torch.save(model.state_dict(), f"checkpoints/mnist-linear-second/model_checkpoint_epoch_{epoch+1}.pth")    
    ...
    with torch.no_grad():        
        ...
        #generated_samples = diffusion.sample_ddpm(x_shape=(1, 1, 32, 32), device=device, labels=single_label)                
        ...
        visualize_denoising_process(generated_samples, epoch=epoch, path="samples/generated_samples/mnist-linear-second", filename="samples_epoch", image_title="Denoising Process Visualization")


# Save the trained model
torch.save(model.state_dict(), "checkpoints/mnist-linear-second/model.pth")
