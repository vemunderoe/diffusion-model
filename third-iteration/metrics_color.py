...
#model = UNet(in_channels=3, out_channels=3, base_channels=64, embedding_dim=128).to(device) second_unet.py
model = UNet(in_channels=3, out_channels=3, base_channels=64, embedding_dim=128, num_classes=10).to(device) # unet.py
...
def sample_images(num_samples=100, batch_size=10):
    ...
    with torch.no_grad():
        ...
        for i in range(num_full_batches):
            sample_batch = diffusion.sample(x_shape=(batch_size, 3, 32, 32), device=device, labels=torch.tensor([7], device=device))
            ...
            save_image(generated_imgs, f'isfid/generated_images_{i + 1}.png')
            samples.extend([resize_transform(img) for img in generated_imgs])

        # Generate the remainder if necessary
        if remainder > 0:
            sample_batch = diffusion.sample(x_shape=(remainder, 3, 32, 32), device=device, labels=torch.tensor([i % 10], device=device))
...
for i in range(num_samples):
    ...
    save_image(generated_images[i], f'isfid/generated_images_{i + 1}upscaled.png')
