...
model.load_state_dict(torch.load("checkpoints/cifar10-new-unet-linear/model_checkpoint_epoch_201.pth", map_location=device, weights_only=True))
model.eval()
...
def sample_images(num_samples=100, batch_size=10):
    ...
    with torch.no_grad():
        ...
        for _ in range(num_full_batches):
            sample_batch = diffusion.sample_ddpm(x_shape=(batch_size, 3, 32, 32), device=device)
        ...
        if remainder > 0:
            sample_batch = diffusion.sample_ddpm(x_shape=(remainder, 3, 32, 32), device=device)
